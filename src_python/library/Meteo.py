...
def GetEmu1Vars(fname, tstart, tend, ttol=3600., start_offset=0, end_offset=0):
    ...
    dn = - \
        999.        # In-cloud aerosol number loss tendency = change in interstitial aerosol+cloud droplet number concentration in cloudy columns (#/kg/s)
    ...
    i = 0
    for t in times:
        if abs(t - tstart) < abs(times[ind_tstart] - tstart):
            ind_tstart = i
        if abs(t - tend) < abs(times[ind_tend] - tend):
            ind_tend = i
        i += 1
    #
    ...
    if 'Nc_ic' in ncid.variables:    # Level 4 = SALSA microphysics
        ...
        if ncfrac >= 2:  # Linear fit needs at least two data points
            ...
            # + interstitial aerosol (domain mean)
            nc += ncid.variables['Na_int'][ind_tstart:ind_tend]
...
def get_netcdf_updraft(fname, tstart, tend, ttol=3600., tol_clw=1e-5):
    ...
    # Mean cloud droplet number concentration at the cloud base with positive updraft velocity (1/kg)
    cdnc_p = 0.
    # Velocity weigted mean cloud droplet number concentration at the cloud base with positive updraft velocity (1/kg)
    cdnc_wp = 0.
        ...
        for i in range(0, dims[0]):
            for j in range(0, dims[1]):
                ...
                if len(kk) > 0 and rflx_calc:
                    ...
                    m += 1
                    k = kk[0]
                    h = kk[-1]  # The first and last cloudy grid cell
...
def get_netcdf_variable(fname, var_name, start_time, end_time=-10000., ttol=3600., start_offset=0, end_offset=0):
    ...
    if end_time > -9999.:
        ...
        for tt in times:
            # Closest match
            if abs(tt - start_time) < abs(times[ind_start] - start_time):
                ind_start = i
            if abs(tt - end_time) < abs(times[ind_end] - end_time):
                ind_end = i
            i += 1
        #
        ...
        for tt in times:
            # Closest match
            if abs(tt - start_time) < abs(times[ind] - start_time):
                ind = i
            i = i + 1
        #
...
def GetEmu2Vars(path, prefix):
    ...
    while True:
        ...
        if fileCSV.is_file():
            ...
            wpos, w2pos, cdnc_p, cdnc_wp, n, drflx, m = get_netcdf_updraft(
                file_4d, tstart, tend, ttol=10.)
...
def extract_write_data(fname_template, specs, name_out='', nmax=9999, skip_errs=False):
        ...
        for tt in cmd:
            if (tt == '[' or tt == ')' or tt == ',') and lst == -1:
                ...
                if frst == 0:
                    ...
                    fun = cmd[:frst + 1] + 'ncid.variables[\'' + \
                        cmd[frst + 1:lst] + '\']' + cmd[lst:]
                return fun
            elif tt == '(' or i == 0:
                frst = i
            i += 1
    ...
    if len(name_out):
        ...
        if nerr > 0:
            print('   ' + str(nerr) + ' error(s) ignored: ', msg)
    #
    # Return the data
    return out
