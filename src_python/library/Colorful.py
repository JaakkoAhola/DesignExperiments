...
def get_indy_color_list(colorNumber=None,
    ...
    nmax = 22 - int(not use_white) - int(not use_black) - int(not use_beige) - \
        int(not use_lavender) - int(not use_grey)

    if snsColorPalette is None and matplotlibColorMap is None and colorNumber < nmax:
        colorList = get_distinct_color_list_by_number(colorNumber,
                                                      blindness_level=blindness_level,
                                                      use_white=use_white,
                                                      use_black=use_black,
                                                      use_beige=use_beige,
        ...
        colorList = [matplotlibColorMap(i)
                     for i in numpy.linspace(0, matplotlibLimiter, colorNumber)]
...
def get_distinct_color_list_by_name(elements):
    distinct_colors_df = get_distinct_colors_df()
...
""").df()
    ...
    if elements > len(color_list_all_possible) and (blindness_level > 1):
        return get_distinct_color_list_by_number(elements,
                                                 blindness_level - 1,
                                                 use_black=use_black,
                                                 use_white=use_white,
                                                 use_lavender=use_lavender,
    ...
    # if the number of colors needed is larger than possible colors and blindness_level can't be decreased, return False value
        ...
        # give list of distinctColors with given number, blindness_level and choice of using white & black
        color_list = color_list_all_possible[:elements]
