...
def setXLabels(ax, ticks, shownLabels=None, start=0, end=8, interval=2, integer=True):
    shownLabelsBoolean = _setLabels(
        ax.set_xticklabels, ax.xaxis, ticks, shownLabels, start, end, interval)
    return shownLabelsBoolean


def setYLabels(ax, ticks, shownLabels=None, start=0, end=8, interval=2, integer=True):
    shownLabelsBoolean = _setLabels(
        ax.set_yticklabels, ax.yaxis, ticks, shownLabels, start, end, interval)
    return shownLabelsBoolean
...
def _setLabels(axset, ax_axis, ticks, shownLabels=None, start=0, end=8, interval=2, integer=False):
    ...
    hideLabels(ax_axis, shownLabelsBoolean)
...
def hideXTickLabels(ax):
    _hideAllTickLabels(ax.get_xticklabels)


def hideYTickLabels(ax):
    _hideAllTickLabels(ax.get_yticklabels)
...
def setXTickSizes(ax, labelListMajorLineBoolean,
    ...
    _setTickSizes(ax.xaxis, labelListMajorLineBoolean, majorFontsize, minorFontSize)
...
def setYTickSizes(ax, labelListMajorLineBoolean,
    ...
    _setTickSizes(ax.yaxis, labelListMajorLineBoolean, majorFontsize, minorFontSize)
...
def getMathLabel(label):
    ...
    mathlabel = getMathLabelFromDict(label)
    if "{" in mathlabel:
        mathlabel = getLatexLabel(mathlabel)
    return mathlabel


def getMathLabelTableFormat(label):
    return "$" + getMathLabelFromDict(label) + "$"
...
def getMathLabelSubscript(label):
    ...
    try:
        subscript = getMathLabelFromDict(label).split(
            "_")[1].replace("{", "").replace("}", "")
    except IndexError:
        subscript = ""
...
def setXaxisLabel(ax, label, unit=None, useBold=True):
    _setLabel(ax.set_xlabel, label, unit, useBold)


def setYaxisLabel(ax, label, unit=None, useBold=True):
    _setLabel(ax.set_ylabel, label, unit, useBold)


def _setLabel(labelPointer, label, unit, useBold):
    if unit is not None:
        label = getUnitLabel(label, unit, useBold)
...
def setLegendSimulation(ax, simulationList, loc="center"):
    ...
    setLegend(ax, collectionOfLabelsColors, loc)
...
def setArtist(ax,
    ...
    artist = ax.legend(handles=legend_elements, loc=loc, frameon=True,
                       framealpha=framealpha, fontsize=fontsize, ncol=ncol)
